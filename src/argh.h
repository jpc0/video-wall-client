...
namespace argh
{
    ...
    public:
        ...
        explicit parser(std::initializer_list<char const *const> pre_reg_names)
        {
            add_params(pre_reg_names);
        }

        explicit parser(const char *const argv[], int mode = PREFER_FLAG_FOR_UNREG_OPTION)
        {
            parse(argv, mode);
        }
    ...
    private:
        static string_stream bad_stream();
        static std::string trim_leading_dashes(std::string const &name);
        static bool is_number(std::string const &arg);
        static bool is_option(std::string const &arg);
    ...
    inline string_stream parser::bad_stream() 
    {
        string_stream bad;
    ...
    inline bool parser::is_number(std::string const &arg) 
    {
    ...
    inline bool parser::is_option(std::string const &arg) 
    {
        assert(0 != arg.size());
        if (is_number(arg))
            return false;
    ...
    inline std::string parser::trim_leading_dashes(std::string const &name) 
    {
